 ...
 */
export const readAndWrite = (file, name, originPath, newPath) => {
    ...
    try {
        ...
        fs.renameSync(`${newPath}/${file}`, `${newPath}/${file.replace("$name", name)}`);
 ...
 */
export const makeDirAndWriteRecursive = (originPath, newPath, name) => {
    try {
    ...
    files.forEach(file => {
        ...
        if (fs.statSync(newOrigin).isDirectory()) {
            makeDirAndWriteRecursive(newOrigin, `${newPath}/${file.replace("$name", name)}`, name);
        }
        else {
